...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        value = do_lambda_form(new_expressions, env)
        env.bindings[name] = value
        return name
        # END Question 9A
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN Question 16
    def make_bindings(bindings, formals, vals):
        if bindings is nil:
            return formals, vals
        elif scheme_symbolp(bindings.first.first):
            check_form(bindings.first, 2,2)
            binding = bindings.first
            vals = Pair(scheme_eval(binding.second.first, env), vals)
            formals = Pair(binding.first, formals)
            return make_bindings(bindings.second, formals, vals)
        else:
            raise SchemeError
    formals, vals = make_bindings(bindings,nil,nil)
...
def check_formals(formals):
    ...
    while formals is not nil:
        if formals.first in formals_checker:
            raise SchemeError("symbol already repeated in list: " + str(formals.first))
        elif not scheme_symbolp(formals.first):
            raise SchemeError(str(formals.first) + " not a symbol")
        else:
            formals_checker += [formals.first]
        formals = formals.second
    # END Question 11B
