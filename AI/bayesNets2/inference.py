...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for eliminator in eliminationOrder:
            notJoined, joined = joinFactorsByVariable(factors, eliminator)
            if len(joined.unconditionedVariables()) == 1:
                factors = notJoined
            else:
                eliminated = eliminate(joined, eliminator)
                factors = notJoined + [eliminated]
        if len(factors) == 1:
            toBeNormalized = factors[0]
        else:
            toBeNormalized = joinFactors(factors)
        return normalize(toBeNormalized)
