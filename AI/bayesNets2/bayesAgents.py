...
def constructBayesNet(gameState):
    ...
    for obs in obsVars:
        edges += [(FOOD_HOUSE_VAR, obs)]
        edges += [(GHOST_HOUSE_VAR, obs)]
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    foodHouseInf = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    mostLikely = 0
    mostProp = 0
    for assignment in foodHouseInf.getAllPossibleAssignmentDicts():
        if foodHouseInf.getProbability(assignment) > mostProp:
            mostLikely = assignment
            mostProp = foodHouseInf.getProbability(assignment)
    return mostLikely
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        """
        foodLeftGhostRightProb = 0
        foodRightGhostLeftProb = 0
        houseVarInf = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        for assignment in houseVarInf.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                foodLeftGhostRightProb += houseVarInf.getProbability(assignment)
            elif assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                foodRightGhostLeftProb += houseVarInf.getProbability(assignment)

        leftExpectedValue = foodLeftGhostRightProb * WON_GAME_REWARD + foodRightGhostLeftProb * GHOST_COLLISION_REWARD
        rightExpectedValue = foodLeftGhostRightProb * GHOST_COLLISION_REWARD + foodRightGhostLeftProb * WON_GAME_REWARD
