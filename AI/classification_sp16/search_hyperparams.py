...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        paramDescentSolver = solvers.MinibatchStochasticGradientDescentSolver(learning_rate, iterations, batch_size, momentum)
        paramDescentSolver.solve(train_data, train_labels, val_data, val_labels, model)
        val_accuracies.append(model.accuracy(val_data, val_labels))
        if model.accuracy(val_data, val_labels) == max(val_accuracies):
            best_model = model
