...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        while i < len(grad_tensors):
            param_op1 = tf.multiply(self.learning_rate, grad_tensors[i])
            param_op2 = tf.subtract(param_vars[i], param_op1)
            updates.append((param_vars[i], param_op2))
            i += 1
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            train_loss, updated_vals = session.run([loss_tensor, update_ops], feed_dict=train_dict)
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        val_losses = []
        for iter_ in range(self.iterations):
            train_time = train_gen.next()
            
            train_loss, updated_vals = session.run([loss_tensor, update_ops], feed_dict={placeholders[0]: train_time[0], placeholders[1]: train_time[1]})
            val_time = val_gen.next()
            
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: val_time[0], placeholders[1]: val_time[1]})
